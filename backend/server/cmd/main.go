...
func run() error {
	...
	defer search.Close()
	app := fiber.New(fiber.Config{DisableStartupMessage: true, BodyLimit: 1024, ReadTimeout: 10 * time.Second, WriteTimeout: 10 * time.Second, IdleTimeout: 30 * time.Second})
